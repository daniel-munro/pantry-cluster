...
N = nrow(wgtlist)
## Allow for 'chr1' chromosome format used by GTEx annotations -Daniel Munro
...
out.tbl = data.frame( "PANEL" = rep(NA,N) , "FILE" = character(N) , "ID" = character(N) , "CHR" = character(N) , "P0" = character(N) , "P1" = character(N) ,"HSQ" = numeric(N) , "BEST.GWAS.ID" = character(N) , "BEST.GWAS.Z" = numeric(N) , "EQTL.ID" = character(N) , "EQTL.R2" = numeric(N) , "EQTL.Z" = numeric(N) , "EQTL.GWAS.Z" = numeric(N) , "NSNP" = numeric(N) , "NWGT" = numeric(N) , "MODEL" = character(N) , "MODELCV.R2" = character(N) , "MODELCV.PV" = character(N) , "TWAS.Z" = numeric(N) , "TWAS.P" = numeric(N) , stringsAsFactors=FALSE )

## Output empty table and exit if no weights found for this chr -Daniel Munro
if (N == 0) {
	cat("No weights found for chromosome ", opt$chr, ". Saving empty output table.\n")
...
## Use chromosome format of GTEx annotations: -Daniel Munro
# mhc = as.numeric(out.tbl$CHR) == 6 & as.numeric(out.tbl$P0) > 26e6 & as.numeric(out.tbl$P1) < 34e6
mhc = out.tbl$CHR == "chr6" & as.numeric(out.tbl$P0) > 26e6 & as.numeric(out.tbl$P1) < 34e6
