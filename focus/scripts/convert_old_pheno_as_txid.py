...
def import_fusion(path, name, tissue, assay, use_ens_id, from_gencode, rsid_table, session):
    ...
    # For Pantry, IDs are actually phenotype IDs, gene ID followed by a sequence starting with {:,.}
    # Extract gene IDs and add modality to phenotype IDs so it's present in results
    fusion_db['gene_id'] = fusion_db.ID.str.extract(r"([^:.]+)")
    fusion_db['modality'] = fusion_db.WGT.str.extract(r"([^/]+)")
    fusion_db['pheno_id'] = fusion_db['modality'] + ':' + fusion_db['ID']
    genes = fusion_db.gene_id.unique()
    ...
    for rdx, row in fusion_db.iterrows():
        ...
        wgt_name, g_name, pheno_id, chrom, txstart, txstop = row.WGT, row.gene_id, row.pheno_id, row.CHR, row.P0, row.P1
        ...
        gene_info["txid"] = pheno_id # Store phenotype ID so it's present in results
